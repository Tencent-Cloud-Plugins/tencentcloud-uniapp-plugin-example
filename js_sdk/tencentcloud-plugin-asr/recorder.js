    ...
    return new Promise((resolve, reject) => {
      ...
      this.rec.stop(
        async (blob, duration) => {
          ...
          voiceBase64 = (/.+;\s*base64\s*,\s*(.+)$/i.exec(voiceBase64) || [])[1];
      ...
      this.rec.onStop(async ({ tempFilePath, fileSize, duration }) => {
        ...
        // #endif
        // #ifdef APP-PLUS
        const res = await filePath2Base64(tempFilePath);
        const voiceBase64 = (/.+;\s*base64\s*,\s*(.+)$/i.exec(res.voiceBase64) || [])[1];
        resolve({ voiceBase64, size: res.size });
        // #endif
      });
      this.rec.stop();
      // #endif
